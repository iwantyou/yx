...
const { readFile, readTemplate, writeFile, sourceNormal, router } = require("./tils");
let spin = ora();
// 序列化参数
...
yargs(process.argv.slice(2))
  ...
  .command("init <name>", "在指定仓库上拉取代码并写入配置", {}, init)
  .command("generate [name]", "生成配置模版config.json", {}, generate)
  .help()
  .alias("h", "help")
  .version("1.0.0")
...
function init(argv) {
  ...
  gitClone({ url: repoargv.url, targetPath: repoargv.project }, async (err) => {
    ...
    try {
      ...
      // 配置router && 生成目录
      let routers = writeFile(routerPath, router(data, path.join(repoargv.project, "src/pages")), "w+");
